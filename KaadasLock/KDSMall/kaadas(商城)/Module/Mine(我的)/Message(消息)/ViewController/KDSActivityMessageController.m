//
//  KDSActivityMessageController.m
//  kaadas
//
...
@interface KDSActivityMessageController ()
<
UITableViewDelegate,
...
@implementation KDSActivityMessageController
...
#pragma mark - 下拉刷新
-(void)activatyHeaderRefresh{
    _pageNum = 1;
    [self activityMessageRequest];
}
#pragma mark - 上拉加载更多
-(void)activatyFooterRefresh{
    _pageNum ++;
    [self activityMessageRequest];
}
...
-(void)activityMessageRequest{
    NSString * userToken = [[NSUserDefaults standardUserDefaults] objectForKey:USER_TOKEN];
    NSDictionary * dictionary = @{@"params": @{ @"messageType":@"message_type_activity"},
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    KDSActivityMessageCell * cell = [KDSActivityMessageCell activityMessageCellWithTableView:tableView];
...
#pragma mark - 创建UI
-(void)createUI{
    self.navigationBarView.backTitle =@"活动消息";
...
-(UITableView *)tableView{
    if (_tableView == nil) {
        ...
        _tableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingTarget:self refreshingAction:@selector(activatyHeaderRefresh)];
        _tableView.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingTarget:self refreshingAction:@selector(activatyFooterRefresh)];
