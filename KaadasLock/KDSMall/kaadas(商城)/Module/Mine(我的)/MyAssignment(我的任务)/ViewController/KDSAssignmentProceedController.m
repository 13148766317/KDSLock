//
//  KDSAssignmentProceedController.m
//  kaadas
//
...
@interface KDSAssignmentProceedController ()
<
UITableViewDelegate,
...
@implementation KDSAssignmentProceedController
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.cellType = KDSAssignmentCellType_proceed;
...
#pragma mark - 下拉刷新
-(void)assignmentHeaderRefresh{
    _pageNum = 1;
    [self requestData];
}

#pragma mark - 上拉加载更多
-(void)assignmentFooterRefresh{
    _pageNum ++;
    [self requestData];
}
...
-(void)requestData{
    ...
    [KDSMineHttp myTaskongoingWithParams:paramDict success:^(BOOL isSuccess, id  _Nonnull obj) {
        if (isSuccess) {
            ...
            if (isSuccess) {
                ...
                KDSMyTaskOnGoingmodel * model = (KDSMyTaskOnGoingmodel *)obj;
...
-(UITableView *)tableView{
    if (_tableView == nil) {
        ...
        _tableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingTarget:self refreshingAction:@selector(assignmentHeaderRefresh)];
        _tableView.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingTarget:self refreshingAction:@selector(assignmentFooterRefresh)];
