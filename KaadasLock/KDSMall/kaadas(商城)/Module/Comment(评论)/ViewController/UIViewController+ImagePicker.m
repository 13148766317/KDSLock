...
- (void)presentChoseActionSheet {
    ...
    UIAlertAction * takePhotoAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"相机", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
            if (granted) {
                ...
                UIAlertController * noticeAlertController = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"无相机权限，请到设置中打开", nil) message:nil preferredStyle:UIAlertControllerStyleAlert];
                UIAlertAction * cancelAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"取消", nil) style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
                    
                }];
                
                UIAlertAction * okAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"现在就去", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                    //跳转到设置界面
    ...
    UIAlertAction * choseFromAlbumAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"相册", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        //判断相册权限
        [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
            if (status == PHAuthorizationStatusNotDetermined || status == PHAuthorizationStatusAuthorized) {
                ...
                UIAlertController * noticeAlertController = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"无相册权限，请到设置中打开", nil) message:nil preferredStyle:UIAlertControllerStyleAlert];
                UIAlertAction * cancelAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"取消", nil) style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
...
//    self.completionHandler(imageData, image);
    ...
    if ([type isEqualToString:@"public.image"])
    {
        ...
        imagesize.width =600;
        image=[self reSizeImage:image toSize:imagesize];
