...
//        NSArray *array  =[self JsonObject:@"alladdress.json"];
        
        NSData *JSONData = [NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"alladdress" ofType:@"json"]];
        ...
        NSString *jsonStr2 = dataDic[@"data"];
        NSMutableArray *dataArr = [self arrayWithJsonString:jsonStr2];
        ...
        [dataArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            AProvinceModel *model = [AProvinceModel showDataWith:obj];
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component
{
    ...
    if (component == 0) {
        ...
        AProvinceModel *model = self.shengArray[self.selectRowWithProvince];
    ...
    }else if (component == 2)
    {
        ...
        if (model.city.count <= 0) {
            ...
            ACityModel *cityModel = model.city[self.selectRowWithCity];
...
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component
{
    switch (component) {
        case 0:{
            AProvinceModel *model = self.shengArray[row];
            return model.name;
        }
            break;
        case 1:{
            AProvinceModel *model = self.shengArray[self.selectRowWithProvince];
            ACityModel *citymodel = model.city[row];
        ...
        case 2:
        {
            AProvinceModel *model = self.shengArray[self.selectRowWithProvince];
            ACityModel *citymodel = model.city[self.selectRowWithCity];
            AAreaModel *teanmodel = citymodel.area[row];
...
- (void)determineBtnAction:(UIButton *)button
{
    ...
    AProvinceModel *provincemodel = self.shengArray[shengRow];
    
    ACityModel *cityModel = nil;
    AAreaModel *areaModel = nil;
    
    if (provincemodel.city.count > 0) {
         cityModel = provincemodel.city[shiRow];
         areaModel = cityModel.area[xianRow];
    }
//    NSLog(@"省=%@",provincemodel.name);
//    NSLog(@"省id=%@",provincemodel.code);
//    NSLog(@"市=%@",cityModel.name);
//    NSLog(@"市id=%@",cityModel.code);
//    NSLog(@"区=%@",areaModel.name);
//    NSLog(@"区id=%@",areaModel.code);

    
    NSDictionary *dic1 = @{@"name":provincemodel.name,@"id":provincemodel.code};
    NSDictionary *dic2 = nil;
    if (cityModel) {
        dic2 = @{@"name":cityModel.name,@"id":cityModel.code};
    }else{
        dic2 = @{@"name":@"",@"id":@""};
    }
    ...
    if (areaModel) {
       dic3  = @{@"name":areaModel.name,@"id":areaModel.code};
    }else{
        dic3 = @{@"name":@"",@"id":@""};
    }
    ...
    NSMutableArray *arr = [NSMutableArray arrayWithObjects:dic1,dic2,dic3, nil];
    if (self.cityBlock) {
        self.cityBlock(arr);
    }
