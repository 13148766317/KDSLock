...
typedef struct _FECRecover
{
	...
	uint32_t flag;	
	uint32_t continuity;
	//mblk_t *m[RTP_FECPROTECTDATA_NUM+RTP_FECPKG_NUM+1];
	//uint16_t seq[RTP_FECPROTECTDATA_NUM+RTP_FECPKG_NUM+1];
	//uint16_t fecseq[RTP_FECPKG_NUM];//¨¢???¨¢?D?fec¡ã¨¹¦Ì?D¨°o?
...
typedef struct _RtpStream
{
	...
	uint16_t fecsnd_seq;/* fec send sequence number */
...
ORTP_PUBLIC mblk_t * rtp_session_create_packet_for_fec(RtpSession *session,size_t header_size, const uint8_t *payload, size_t payload_size,size_t mini_size,int extsize);
ORTP_PUBLIC mblk_t * rtp_session_create_fec_packet(RtpSession *session,size_t payload_size,int i);
