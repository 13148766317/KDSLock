...
-(void)getAllBindDevice
{
    ...
    __weak typeof(self) weakSelf = self;
    self.mqttReceipt = [[KDSMQTTManager sharedManager] getGatewayAndDeviceList:^(NSError * _Nullable error, NSArray<GatewayModel *> * _Nullable gws, NSArray<MyDevice *> * _Nullable bles, NSArray<KDSWifiLockModel *> * _Nullable wifiList, NSArray<KDSProductInfoList *> * _Nullable productInfoList) {
        ...
        for (KDSWifiLockModel * wifiDevice in wifiList) {
            ...
            if (index == NSNotFound)
            {
                [weakSelf.devicesArr addObject:wifiDevice];
            }
            else
            {
